...
2. Issue with broken links: Navbar didn't respond, it didn't move to each section when clicked on individual links.
 The error appeared because I implemented the solution to close dropdown menu after selecting link on navbar on small devices
...
also when clicked on logo-brand. I wasn't sure how to proceed this issue. I tried to find the solution on the internet and also on the slack, but unfortunatelly given
...
- There was also one undefined bootstrap variable which was used for hide collapsible navbar on click in bootstrap5. I try to removed but after that code did not work.
...
- On the Contact page there is a contact form which user can fill in send. The contact form is connected using EmailJS service which allows users
to send their email directly without using the server. It was used the alert as interactive custom modal popup window when a user submits a contact form.
Depending on if the user puts data in the form it will display success message 
